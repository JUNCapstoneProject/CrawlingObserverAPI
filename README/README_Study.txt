...
25.05.31. ~ 25.06.06.
1. git
- 기존의 브랜치 하나로 작업 후 pr 하는 방식은 로그가 더러워지는 문제 발생
- 매 작업마다 브랜치를 생성 후 커밋하고, pr 이후 병합이 끝난 브랜치는 지우는 방식을 일반적으로 사용한다는 것을 알게 됨
- 각 작업의 목적을 브랜치의 이름으로 설정하면 버전 관리가 더 용이해진다는 것을 알게 됨
- 해당 작업을 자동화하는 스크립트를 작성하여 더 가시성이 좋은 로그를 남기는 방식으로 수정함
2. 로거
- 기존의 직접 작성한 로거에서 더 범용적으로 사용하는 logging 라이브러리로 변경함
- 콘솔과 파일에 사용하는 핸들러를 분리하여 출력/저장되는 로깅 레벨을 구분함
- 라이브러리가 사용하는 하위 패키지들의 로거까지 커스텀 로거로 설정되어 필요 없는 메세지가 뜨는 이슈가 생김
- 내가 작성한 프로젝트 내에서만 커스텀 로거를 사용하도록 수정함
3. 크롤러
- 중복 데이터 처리를 위해 주가 정보를 저장할 때, company와 posted at의 조합을 검사하는 로직 추가
- db의 반환값은 datetime인데 저장 전 데이터는 date라서 제대로 필터링이 안되는 문제 발생
- 형식을 맞춰 검사하는 방식으로 수정
- 교수님 피드백에 따라서 바뀐 DB 구조에 대응하기 위해 모델을 db 수정 내용에 맞게 고치고, 저장로직을 담당하는 핸들러도 수정함
4. 협업
- 크롤링 데이터 분석 요청 간 필요한 데이터 누락으로 에러가 발생함
- 공유한 코드 간의 차이점으로 인해 누락된 이슈였음(종목 및 시장에 대한 티커 정보)
- 해당 데이터를 충족하기 위해 코드를 수정하고 다시 테스트하여 성공함
- ai 분석 api는 요청을 받으면 메세지 큐에 넣고 처리하는 방식인데, 너무 많은 요청으로 대기큐가 병목되는 현상이 발생함(한다고 들음)
- 기존에 요청 했거나 요청한지 얼마 안된 데이터를 미리 필터링 하고 요청을 보내는 방식으로 수정함
- 재무제표 데이터 분석 시 데이터가 없으면 null로 패딩하여 보내는 방식은 분석 결과를 도출하지 못하고 에러를 발생시켰음
- 분석 후보를 고르는 과정에서 필요한 데이터가 하나라도 있으면 제외하는 방식으로 수정함
- ai 분석 결과를 받고 db에 저장할 때 response 형식에 맞추어 기존의 string 형식에서 int 형식으로 수정함
5. DB
- unique 제약조건이 걸려있지 않아 중복 데이터가 들어가던 문제를 수정함
- 난잡한 외래키 설정을 수정하여 일관성 있는 외래키로 재설정 하여 관리 용이성을 더함
- 확장된 db 구조에 맞추어 관리 용이성을 위해 각 테이블마다 고유 pk를 가지도록 설정함
- 뉴스/리포트와 각각의 태그 테이블의 관계가 1:n 관계에서 1:1 관계로 변경되어, 기존의 crawling_id로 연결된 관계에서 각 news/reports 테이블이 tag_id를 참조하는 방식으로 변경
- 각 테이블과 뷰의 사용처를 명확히 하여 구분하기 편하게 ERD를 재 작성함
- DB의 참조 관계와 컬럼 등의 변화로 인해 기존에 사용하던 뷰에 발생한 오류들을 해결함
6. assist 백엔드
- 분석 결과의 반환값이 string -> integer로 변경됨에 따라 백엔드에서 해당 내용을 처리하던 부분의 데이터 형식을 맞춤
- 데이터 형식의 변화에 따라 entity, service, repository 등의 처리 로직도 수정함
- news 데이터를 페이지 슬라이싱 할 때, 별도의 기준이 없어 중복 데이터가 다른 페이로 들어가는 문제 발생
- 뉴스의 관련 종목과 게시 날짜를 기준으로 정렬한 후 필터링된 내용을 슬라이스 하는 방식으로 변경하여 해결함